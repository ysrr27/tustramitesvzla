              ...
              <div class="x_panel">
                ...
                <div class="x_content">
                  ...
                  <table class="table table-striped projects">
                    ...
                    <tbody>
                <?php
                $SQL="SELECT * FROM m_ventas, m_clientes, m_ofertas WHERE m_venta_idOferta='$idOferta' AND m_venta_idUsuario=m_cliente_id AND m_venta_idOferta=m_oferta_id ORDER BY m_venta_fecha ASC";
                      ...
                      <tr id="Venta<?=$m_venta_id?>">
                        ...
                        <td>
                         <?php if (control_access("VENTAS", 'ELIMINAR')) { ?>
                          <button type="button" class="btn btn-success btn-xs" <?php if($m_venta_estatus!="P"){ echo "disabled"; }?> data-id="<?=$m_venta_id?>" data-accion="A" data-title="Seguro que desea Aprobar esta venta?" data-trigger="focus" data-on-confirm="changeStatus" data-toggle="confirmation" data-btn-ok-label="Sí" data-btn-cancel-label="Cancelar!" data-placement="top" title="Seguro que desea Aprobar esta venta?">  <i class="fa fa-check-square"> </i> Aprobar</button>
                          <?php } ?>
                           <?php if (control_access("VENTAS", 'ELIMINAR')) { ?>
                          <button type="button" class="btn btn-danger btn-xs" <?php if($m_venta_estatus!="P"){ echo "disabled"; }?>  data-id="<?=$m_venta_id?>" data-accion="R" data-title="Seguro que desea Rechazar esta venta?" data-trigger="focus" data-on-confirm="changeStatus" data-toggle="confirmation" data-btn-ok-label="Sí" data-btn-cancel-label="Cancelar!" data-placement="top" title="Seguro que desea rechazar esta venta?">  <i class="fa fa-trash-o"> </i> Rechazar</button>
                          <?php } ?>
                        </td>
                      </tr>
               <?php } ?>
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['success']) {
        ...
        setTimeout(function() { window.location.href = 'index.php?id=<?=$m_venta_id?>';}, 1000);
      }
      else{
        $("#mensajes").css("z-index", "999");

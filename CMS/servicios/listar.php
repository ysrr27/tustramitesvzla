...
if ($cantidadResultados=="") {
  $SQL="SELECT * FROM m_servicios ORDER BY m_servicio_estatus ASC, m_servicio_nombre ASC LIMIT 0,20";
}
else{
  $SQL="SELECT * FROM m_servicios WHERE 1=1  ";
  if ($titulo!="") {
   $SQL.=" AND CONCAT( m_servicio_nombre, '', m_servicio_descripcion)LIKE '%$titulo%'";
 }

 if ($estatus!='x') {
   $SQL.=" AND m_servicio_estatus='$estatus'";
 }
 if ($cantidadResultados!="") {
                ...
                <div class="x_content">
                  ...
                  <tbody>
                    ...
                    while ($row=mysqli_fetch_array($query)) {
                      ...
                      $m_servicio_created_at=date("d/m/Y h:i:s", strtotime($m_servicio_created_at));
                      $m_servicio_updated_at=date("d/m/Y h:i:s", strtotime($m_servicio_updated_at));
                  ...
                  ?>
                  <tr id="Servicio<?=$m_servicio_id?>">
                    <td>#</td>
                    <td>
                      <a><?=$m_servicio_nombre?></a>
                    </td>
                    <td>
                      <ul class="list-inline">
                        <li>
                         <i class="fa f<?=$m_servicio_icono?> fa-2x picker-target"></i>
                        </li>
                    ...
                    <td>
                      <a href="../ventas/index.php?id=<?=$m_servicio_id?>" class="btn btn-primary btn-xs"><i class="fa fa-tag"></i> Ventas </a>
                      <a href="editServicio.php?id=<?=$m_servicio_id?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Editar </a>


                      <?php if (control_access("SERVICIOS", 'ELIMINAR')) { ?>
                      <button type="button" class="btn btn-danger btn-xs" data-id="<?=$m_servicio_id?>" data-accion="Eliminar" data-title="Seguro que desea Eliminar?" data-trigger="focus" data-on-confirm="deleteServicio" data-toggle="confirmation" data-btn-ok-label="Sí" data-btn-cancel-label="Cancelar!" data-placement="top" title="Seguro que desea Eliminar?">  <i class="fa fa-trash-o"> </i> Eliminar</button>
                      <?php } ?>

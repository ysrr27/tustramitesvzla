var calendar = {
	...
	init: function(ajax) {
      ...
      $.ajax({
  				url: '/tus_tramitesVzla/process/json/events.json',
  				type: 'GET',
  			})
  			.done(function(data) {
          ...
          for (var i = 0; i < events.length; i++) {
            $('.list').append('<div class="day-event" date-day="'
            					+ events[i].day +'" date-month="' 
            					+ events[i].month +'" date-year="'
            					+ events[i].year +'" data-number="'
            					+ i +'"><a href="#." class="close"><i class="fa fa-times" aria-hidden="true"></i></a><h2 class="title">'
            					+ events[i].title +'</h2><p>'
            					+ events[i].description +'</p></div>');
          }
